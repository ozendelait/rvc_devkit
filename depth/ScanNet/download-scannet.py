...
FILETYPES = ['.aggregation.json', '.sens', '.txt', '_vh_clean.ply',
             '_vh_clean_2.0.010000.segs.json', '_vh_clean_2.ply',
             '_vh_clean.segs.json', '_vh_clean.aggregation.json',
             '_vh_clean_2.labels.ply', '_2d-instance.zip',
...
def main():
    ...
    parser.add_argument('--rob_task_data', action='store_true', help='only download robust '
                        + 'vision challenge task data.')
    parser.add_argument('--label_map', action='store_true', help='whether to download label '
                        + 'map file')
    parser.add_argument('--id', help='specific scan id to download')
    parser.add_argument('--type', help='specific file type to download (.aggregation.json,'
                        + '.sens, .txt, _vh_clean.ply, _vh_clean_2.0.010000.segs.json, _'
                        + 'vh_clean_2.ply, _vh_clean.segs.json, _vh_clean.aggregation.json, '
                        + '_vh_clean_2.labels.ply, _2d-instance.zip, _2d-instance-filt.zip, '
                        + '_2d-label.zip, _2d-label-filt.zip)')
    args = parser.parse_args()

    print('By pressing any key to continue you confirm that you have agreed '
          + 'to the ScanNet terms of use as described at:')
    print(TOS_URL)
    print('***')
    ...
    if args.task_data:  # download task data
        ...
        print('Note that existing scan directories will be skipped. Delete partially '
              + 'downloaded directories to re-download.')
        print('***')
        print('Press any key to continue, or CTRL-C to exit.')
        raw_input('')
